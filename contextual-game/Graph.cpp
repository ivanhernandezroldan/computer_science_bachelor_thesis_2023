...
Graph::Graph(const NetworkData& network) {
	_import_from_networkData(network);
}
...
void Graph::_import_from_networkData(const NetworkData& network)
{

	//1. Reset the members of the class
	clear();

	//2. Start to read information from the network parameter.
	//2.1 The number of vertices of the graph
	m_nVertexNum = network.getNumNodos();

	//2.2 The edges in the graph:
	///   the id of starting point, the id of ending point, the weight of the edge. 
	///   These values are separated by 'white space'. 
	int start_vertex, end_vertex;
	double edge_weight; //TODO podría ser int porque va a ser la variable 'lenght' de la carretera, que está declarada como int
	int vertex_id = 0;

	for (auto carretera : network.getCarreteras()) 
	{
		start_vertex = carretera.init_node;
		end_vertex = carretera.term_node;
		edge_weight = carretera.lenght;

		///2.2.1 Construct the vertices
		BaseVertex* start_vertex_pt = get_vertex(start_vertex);
		BaseVertex* end_vertex_pt = get_vertex(end_vertex);

		///2.2.2 add the edge weight
		...
		///2.2.3 update the fan-in or fan-out variables
		//// Fan-in
